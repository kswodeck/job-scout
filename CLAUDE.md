...
| `.github/workflows/scout.yml` | **PAUSED 2026-08-13** (cron commented out at Kris's request, no end date; `workflow_dispatch` still live — uncomment the `cron:` line to resume). Every-other-day cron `0 9 */2 * *` (odd days of the month, 4:00am CDT / 3:00am CST; back-to-back runs on the 31st→1st of 31-day months; GitHub often fires 1–2h late on quiet repos; nightly until 2026-07-22), installs Claude Code CLI, runs scout, commits `data/`, opens issue |
...
- **Repo is PUBLIC (since 2026-08-12).** Nothing private goes in the tree: `tracker.spreadsheet_id` and `materials.drive_folder_id` are blank in `data/config.json` and read from the `TRACKER_SPREADSHEET_ID` / `MATERIALS_DRIVE_FOLDER_ID` secrets (env wins, config is a fallback for private forks); resume phone/email come from `RESUME_PHONE` / `RESUME_EMAIL` and are absent from `materials/master.js`. Full history was scanned for credentials on 2026-08-12 — clean (the only `sk-ant-` hits are the documented `sk-ant-oat01-` prefix in the docs). `data/applied.json`, `data/matches.json`, and `data/matches.md` are gitignored and live in a **private state repo** (`STATE_REPO` + `STATE_TOKEN` secrets); the workflow clones it in before the run, pushes it back after, and opens the digest issue **there**. Missing secrets = no issue anywhere (fail-safe) and no dedupe for that run. **Caveat: history was not rewritten** (his call, 2026-08-12), so the pre-scrub versions of those files — and the resume phone number — remain readable in old commits.
...
- Feeds are remote-heavy, so **DFW-local hybrid roles are under-covered**. LinkedIn/Indeed prohibit scraping — they stay manual via email alerts (optionally triaged in a Gmail-connected Claude.ai session). Same verdict, re-verified live 2026-07-10, for Wellfound (DataDome-blocked, ToS ban), Built In (public JSON API exists and works, but ToS prohibits automated use), Welcome to the Jungle (job data credential-gated; ToS ban), and Getro-hosted boards like jobs.highfivepartners.com (Getro ToS bans crawling any of its boards; 403s non-browser UAs; that board is mostly non-dev ed-sector roles anyway). ZipRecruiter (checked 2026-07-10): robots.txt is `User-agent: * / Disallow: /`, every non-browser request 403s, no public feed; its job-search API (ZipSearch) is an approved-publisher B2B partner program, not for personal tools → manual via email alerts. Much of its remote/tech inventory is syndicated from ATSes the scout already polls, so the unique loss is mostly DFW-local small-employer postings. **Handshake (`app.joinhandshake.com`, checked 2026-07-15):** the `/job-search` page Kris linked is login-gated and `app.joinhandshake.com/robots.txt` is `Disallow: /`; the only programmatic access is the partner-only EDU API (Career Services / institution-scoped, beta — not available to individuals). There IS a robots-allowed public jobs sitemap (`joinhandshake.com/api/handshake-public-jobs.xml` → per-job `/public/jobs/{id}` HTML pages), but the ToS (`joinhandshake.com/legal/tos/`) expressly bans automated bulk collection: *"does not permit third parties to bulk collect … job descriptions, or other marketplace information … through the use of automated scripts ('scraping'), similar or other technologies."* A sitemap-driven crawler of those pages would still violate the terms — identical situation to Built In → **manual via email alerts, NOT scraped.** (Handshake is also grad/early-career-skewed, so overlap with a 7-yr-senior search is modest.) **UnlistedJobs (`unlistedjobs.com/jobs`, checked 2026-07-17):** signup/paid aggregator that itself scrapes company career pages; origin 403s all non-browser clients (confirmed live on `robots.txt` and `/rss`), no public feed/API; ToS unreadable behind the same bot wall, but a paid data product behind a bot block = the Wellfound verdict → NOT added. Its inventory is company ATS pages the self-growing watchlist already covers directly. **Jobs24x (`jobs24x.com`, checked 2026-07-17): UNVERIFIED, not condemned** — the checking session's sandbox network policy blocked the domain entirely (proxy CONNECT denial), so robots.txt/ToS/feed were unreachable; web search shows a young aggregator of ~1,200 company career pages, no advertised feed/API, middling third-party trust score. Re-check from a normal connection (feed exists? ToS permissive?) before any add; expect heavy overlap with the ATS watchlist regardless.
