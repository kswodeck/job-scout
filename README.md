...
**Not covered (all re-verified live 2026-07-10):** LinkedIn, Indeed, Wellfound, Built In, Welcome to the Jungle, ZipRecruiter, and Getro-hosted boards (e.g. jobs.highfivepartners.com) all prohibit automated access in their ToS and/or block bots — Built In even has a technically perfect public JSON API, but its terms forbid using it, and ZipRecruiter's robots.txt blanket-disallows everything while its job-search API is an approved-publishers-only partner program. They are deliberately excluded. The compliant play: set email job alerts there, then periodically ask Claude (with Gmail connected) to pull the alert emails, dedupe against your tracker, and score them against this same rubric.
